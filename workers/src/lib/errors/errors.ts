...
export const createValidationError = (
  ...
  return new AppError(ErrorCode.VALIDATION_ERROR, message, undefined, 400, {
    field,
    value,
  });
};
...
export const createDatabaseError = (
  ...
  return new AppError(ErrorCode.DATABASE_ERROR, message, undefined, 500, {
...
export const handleDatabaseError = (error: any): AppError => {
  ...
  // Handle common D1 database error patterns
  const errorMessage = error?.message || error?.toString() || '';

  // Check for unique constraint violations
  if (errorMessage.includes('UNIQUE') || errorMessage.includes('unique constraint')) {
    return new AppError(
  ...
  return createDatabaseError(errorMessage || 'Database operation failed', error);
};
...
export const handleNetworkError = (error: any): AppError => {
  ...
  if (error?.name === 'TimeoutError') {
    return new AppError(ErrorCode.TIMEOUT_ERROR, 'Request timeout', undefined, 408);
  }
