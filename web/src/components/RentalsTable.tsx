...
export function RentalsTable() {
  ...
  async function fetchRentals() {
    try {
      const result = await apiClient.listRentals();
  ...
  const handleExtend = async (id: number, currentDueDate: string) => {
    try {
      ...
      await apiClient.extendRental(id, newDueDateStr);
      fetchRentals();
    } catch (error: any) {
      console.error("Failed to extend rental:", error);
      alert(`Failed to extend rental: ${error.message || 'Unknown error'}`);
    }
  };

  const handleReturn = async (id: number) => {
    try {
      await apiClient.returnRental(id);
      fetchRentals();
    } catch (error: any) {
      console.error("Failed to return rental:", error);
      alert(`Failed to return rental: ${error.message || 'Unknown error'}`);
    }
  };
          ...
          {rentals.map((rental) => (
            <tr key={rental.id}>
              ...
              <td className="py-2 px-4 border-b text-center">
                {!rental.returned_at && (
                  <>
                    <Button variant="outline" size="sm" className="mr-2" onClick={() => handleExtend(rental.id, rental.due_date)}>Extend</Button>
